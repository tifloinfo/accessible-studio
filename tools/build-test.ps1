...
if (-not $OutputDirectory) { $OutputDirectory = Join-Path (Split-Path -Parent $root) 'outputs\volume-speech-test-2' }
...
try {
    ...
    Copy-Item -LiteralPath 'RELIABILITY-TEST-NOTES.md' -Destination $OutputDirectory -Force
    $installer = Join-Path $OutputDirectory 'AccessibleStudio-1.1.4-Volume-Speech-Test-2-Setup.exe'
    $hashes = Get-FileHash -LiteralPath @($installer, (Join-Path $root 'package\bin\64bit\accessible-studio.dll')) -Algorithm SHA256 -ErrorAction Stop
